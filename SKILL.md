...
当 `--mode subtitle` 无法下载独立字幕时（B站多数UP主使用硬字幕），自动启用降级：

```
字幕下载失败 → 音频下载(低画质) → Whisper本地转录 → .srt + .txt
```

**前置条件**：安装 `faster-whisper`（`pip install faster-whisper`）
**模型**：默认 `small`（488MB，中文基本准确）
**脚本**：`scripts/whisper_fallback.py`
